...
class FrameBoard(tk.Frame):
 ...
 def D_allow_side(self,auto=False,end_movecode=True):
  ...
  board=self.V_board
  # If bot side:
  if auto and frame_side in "b":
   move=eng.minimax(board,depth=2,side="b",max_="b")
   if move[1] is not None:
    data=move[1]
    self.D_move_tile(self[data[0]],self[data[1].addr],random.choice(list("QRNB")))
    return None
  for addr in self:
   tile=self[addr]
   tile_config={}
