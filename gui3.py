...
class FrameBoard(tk.Frame):
 ...
 def D_allow_side(self):
  ...
  if not has_moves:
   ...
   self.V_holder.V_round.V_undo.config(state='disabled')
   self.V_holder.V_round.V_endgame.config(state='disabled')
 ...
 def D_move_tile(self,source,target):
  ...
  # Other moves
    ...
    if movecode == 'O-O' or  movecode == 'O-O-O':
      if movecode == 'O-O':
 ...
 def D_promotion(self,target,source,piece):
  ...
  self.V_tiles[target.V_loc.addr].config(image=target.V_img)
  self.V_tiles[source.V_loc.addr].config(image = '')
  movecode = movecode = self.V_board[source.V_loc]+(target.V_loc.addr+piece[1])
  target.V_piece=self.V_board[target.V_loc]
  source.V_piece=self.V_board[source.V_loc]
 ...
 def D_load_state(self,state):
  self.D_clear_prestates()
  self.V_board.__init__(state)
  for addr in self.V_tiles:
   tile=self.V_tiles[addr]
...
class ChessTurn(tk.Label):
 def __init__(self,holder,start='w'):
  ...
  self.V_side=self.V_holder.V_holder.V_board.V_side
 ...
 def D_update_side_text(self,*args):
  ...
  # Changes the legal side in the frame board, and clear previous button prestates:
  frameboard=self.V_holder.V_holder.V_board
  frameboard.D_allow_side()
  frameboard.D_clear_prestates()
